...
export default function AdminPage() {
  ...
  // Màn hình chờ khi đang check auth
  if (authChecking) {
    return (
  ...
  if (!currentUser || userRole !== "admin") {
    return (
      <main className="max-w-md mx-auto my-20 p-8 bg-white border border-slate-200 rounded-2xl shadow-sm text-center">
        <div className="w-12 h-12 bg-amber-100 text-amber-700 rounded-full flex items-center justify-center mx-auto mb-4 text-xl font-bold">
          !
        </div>
        ...
        <p className="text-sm text-slate-500 mb-6">
          Trang quản trị này chỉ dành cho tài khoản <strong>Admin</strong>. Tài
          khoản hiện tại của bạn không có quyền thao tác.
        </p>
        <div className="space-x-3">
          <Link
            href="/"
            className="px-4 py-2 text-sm text-slate-600 hover:text-slate-900 border rounded-lg"
          >
            Về trang chủ
          </Link>
          {!currentUser && (
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Đăng nhập với tư cách: {currentUser.email} (Admin)
          </p>
        </div>
        <Link
          href="/"
          ...
          <form onSubmit={handleAddCompetition} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Ví dụ: Thi thử Chuyên Sư Phạm"
                className="w-full p-2.5 border border-slate-200 rounded-lg text-sm outline-blue-600"
                required
              />
            </div>
